...
public class EntityIntrospector {
	private static final Logger log = LoggerFactory
			.getLogger(EntityIntrospector.class);

	private PropertyFilter filter = new PropertyFilter();

	protected String[] deriveGetterName(Field field) {
		log.debug("Derive getter name for field {} from class {}",
	...
	protected String deriveSetterName(Field field) {
		log.debug("Derive setter name for field {} from class {}",
				field.getName(), field.getDeclaringClass().getCanonicalName());
	...
	public Method findGetter(Class<?> beanClass, Field field) {
		...
		if (getterMethod == null) {
			throw new AchillesBeanMappingException("The getter for field '"
					+ fieldName + "' of type '"
	...
	public Method findSetter(Class<?> beanClass, Field field) {
		...
		try {
			...
			throw new AchillesBeanMappingException("The setter for field '"
					+ fieldName + "' of type '"
					+ field.getDeclaringClass().getCanonicalName()
	...
	public String inferColumnFamilyName(Class<?> entity, String canonicalName) {
		...
		if (!StringUtils.isBlank(columnFamilyName)) {
			columnFamilyName = TableNameNormalizer
		...
		log.debug("Inferred columnFamilyName for entity {} : {}",
				canonicalName, columnFamilyName);
		return columnFamilyName;
	}

	public <T> Pair<ConsistencyLevel, ConsistencyLevel> findConsistencyLevels(
			Class<T> entity, AchillesConsistencyLevelPolicy policy) {
		log.debug("Find consistency levels for entity class {}",
	...
	public List<Field> getInheritedPrivateFields(Class<?> type) {
		log.debug(
				"Find inherited private fields from hierarchy for entity class {}",
	...
	public Field getInheritedPrivateFields(Class<?> type, Class<?> annotation) {
		log.debug(
				"Find private field from hierarchy with annotation {} for entity class {}",
		...
		while (i != null && i != Object.class) {
			for (Field declaredField : i.getDeclaredFields()) {
				if (filter.matches(declaredField, annotation)) {
					log.trace("Found inherited private field : {}",
							declaredField);
	...
	public ConsistencyLevel getDefaultGlobalReadConsistency(
	...
	public ConsistencyLevel getDefaultGlobalWriteConsistency(
			AchillesConsistencyLevelPolicy policy) {
