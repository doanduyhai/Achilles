...
public class Validator {
	...
	public static <T> void validateContains(Collection<T> collection,
			T element, String message, Object... args) {
		if (!collection.contains(element)) {
			throw new AchillesException(format(message, args));
		}
	}

	public static void validateBeanMappingNotEmpty(Collection<?> arg,
	...
	public static void validateNotEmpty(Map<?, ?> arg, String message,
			Object... args) {
		if (arg == null || arg.isEmpty()) {
			throw new AchillesException(format(message, args));
		}
	...
	public static void validateNoargsConstructor(Class<?> clazz) {
		if (!clazz.isInterface() && !Modifier.isAbstract(clazz.getModifiers())) {
			...
			throw new AchillesBeanMappingException(format(
					"The class '%s' should have a public default constructor",
	...
	public static void validateRegExp(String arg, String regexp, String label) {
		...
		if (!Pattern.matches(regexp, arg)) {
			throw new AchillesException(format(
					"The property '%s' should match the pattern '%s'", label,
					regexp));
		}
	}
	...
	public static void validateInstantiable(Class<?> arg) {
		...
		try {
			...
			throw new AchillesBeanMappingException(format(
					"Cannot instantiate the class '%s'", canonicalName));
		}
	}

	public static <T> void validateInstanceOf(Object entity,
			Class<T> targetClass, String message, Object... args) {
	...
	public static void validateBeanMappingTrue(boolean condition,
			String message, Object... args) {
		if (!condition) {
			throw new AchillesBeanMappingException(format(message, args));
		}
	...
	public static void validateBeanMappingFalse(boolean condition,
			String message, Object... args) {
		if (condition) {
			throw new AchillesBeanMappingException(format(message, args));
		}
