...
public abstract class TableCreator {
	...
	public void validateOrCreateTables(Map<Class<?>, EntityMeta> entityMetaMap,
		...
		for (Entry<Class<?>, EntityMeta> entry : entityMetaMap.entrySet()) {
			validateOrCreateTableForEntity(entry.getValue(),
					configContext.isForceColumnFamilyCreation());
		}

		if (hasCounter) {
			validateOrCreateTableForCounter(configContext
					.isForceColumnFamilyCreation());
		}
	}

	protected abstract void validateOrCreateTableForEntity(
			EntityMeta entityMeta, boolean forceColumnFamilyCreation);

	protected abstract void validateOrCreateTableForCounter(
