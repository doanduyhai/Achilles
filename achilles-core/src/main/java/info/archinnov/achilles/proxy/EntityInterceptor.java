...
public abstract class EntityInterceptor<CONTEXT extends PersistenceContext, T>
	...
	public Object intercept(Object obj, Method method, Object[] args,
		...
		log.trace("Method {} called for entity of class {}", method.getName(),
		...
		if (idGetter.equals(method)) {
			...
			throw new IllegalAccessException(
					"Cannot change primary key value for existing entity ");
		}
	...
	private <K, V> Object interceptGetter(Method method, Object[] args,
			MethodProxy proxy) throws Throwable {
		Object result = null;
		...
		case COUNTER:
			log.trace(
					"Build counter wrapper for property {} of entity of class {} ",
		...
		case LAZY_LIST:
			if (rawValue != null) {
				...
				result = ListWrapperBuilder.builder(context, list)
						.dirtyMap(dirtyMap).setter(propertyMeta.getSetter())
		...
		case LAZY_SET:
			if (rawValue != null) {
				...
				result = SetWrapperBuilder.builder(context, set)
						.dirtyMap(dirtyMap).setter(propertyMeta.getSetter())
						.propertyMeta(this.getPropertyMetaByProperty(method))
						.proxifier(proxifier).build();
			}
		...
		case LAZY_MAP:
			if (rawValue != null) {
				...
				result = MapWrapperBuilder
						//
						.builder(context, map).dirtyMap(dirtyMap)
		...
		default:
			log.trace(
					"Return un-mapped raw value {} for property {} of entity of class {} ",
					propertyMeta.getPropertyName(),
					propertyMeta.getEntityClassName());
	...
	private Object interceptSetter(Method method, Object[] args,
			MethodProxy proxy) throws Throwable {
