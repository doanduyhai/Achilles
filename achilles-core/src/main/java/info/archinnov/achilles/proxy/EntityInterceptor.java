 ...
 * 
 */
public abstract class EntityInterceptor<CONTEXT extends AchillesPersistenceContext, T> implements MethodInterceptor {
    ...
    private <K, V> Object interceptGetter(Method method, Object[] args, MethodProxy proxy) throws Throwable {
        ...
        switch (propertyMeta.type()) {
            ...
            case JOIN_SIMPLE:
                if (rawValue != null) {
                    log.trace("Build proxy on returned join entity for property {} of entity of class {} ",
                            propertyMeta.getPropertyName(), propertyMeta.getEntityClassName());

                    CONTEXT joinContext = (CONTEXT) context.newPersistenceContext(propertyMeta.joinMeta(), rawValue);
            ...
            case WIDE_MAP:
                if (context.isWideRow()) {
                    log.trace("Build wide row widemap wrapper for property {} of entity of class {} ",
                            propertyMeta.getPropertyName(), propertyMeta.getEntityClassName());
            ...
            case COUNTER_WIDE_MAP:
                ...
                result = buildCounterWideMapWrapper(this.<K, Counter> getPropertyMetaByProperty(method));
                break;
            case JOIN_WIDE_MAP:
