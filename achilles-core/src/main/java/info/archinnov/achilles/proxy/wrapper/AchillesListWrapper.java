...
import info.archinnov.achilles.proxy.wrapper.builder.AchillesListIteratorWrapperBuilder;
import info.archinnov.achilles.proxy.wrapper.builder.AchillesListWrapperBuilder;
 ...
 * 
 */
public class AchillesListWrapper<V> extends AchillesCollectionWrapper<V> implements List<V>
{
	private static final Logger log = LoggerFactory.getLogger(AchillesListWrapper.class);

	public AchillesListWrapper(List<V> target) {
		super(target);
	}
	...
	public ListIterator<V> listIterator()
	{
		...
		return AchillesListIteratorWrapperBuilder //
	...
	public List<V> subList(int from, int to)
	{
		...
		log
				.trace("Build sublist wrapper for list property {} of entity class {} between index {} and {}",
						propertyMeta.getPropertyName(), propertyMeta.getEntityClassName(), from, to);

		return AchillesListWrapperBuilder //
				.builder(context, target)
				//
				.dirtyMap(dirtyMap)
				//
