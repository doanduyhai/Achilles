...
public class EntityLifeCycleListener<CONTEXT extends PersistenceContext> {
	...
	public EntityLifeCycleListener(EntityProxifier<CONTEXT> proxifier2,
	...
	public void intercept(Object entity, Event event) {
		...
		List<EventInterceptor<? extends Object>> eventInterceptors = entityMeta
				.getEventsInterceptor(event);
		if (eventInterceptors.size() > 0) {
			...
			Validator.validateNotNull(entity,
					"The entity class should not be null after interceptor, event:"
							+ event);
			Validator.validateNotNull(entityMeta.getPrimaryKey(entity),
					"The primary key should not be null after interceptor, event:"
							+ event);
		}
