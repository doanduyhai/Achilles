...
public class EntityMetaBuilder {
    ...
    public EntityMeta build() {
        ...
        List<PropertyMeta> eagerMetas = FluentIterable.from(propertyMetas.values()).filter(eagerType).toList();

        meta.setEagerMetas(eagerMetas);
        meta.setEagerGetters(Collections.unmodifiableList(extractEagerGetters(eagerMetas)));

        List<PropertyMeta> allMetasExceptIdMeta = FluentIterable.from(propertyMetas.values()).filter(excludeIdType).toList();
