...
public class AchillesEntityParser {
    ...
    public EntityMeta parseEntity(AchillesEntityParsingContext context) {
        ...
        EntityMeta entityMeta = entityMetaBuilder(idMeta).className(entityClass.getCanonicalName())
                //
                .columnFamilyName(columnFamilyName).serialVersionUID(serialVersionUID)
                .propertyMetas(context.getPropertyMetas()).eagerMetas(context.getEagerMetas())
                .wideRow(context.isWideRow()).consistencyLevels(context.getCurrentConsistencyLevels()).build();
    ...
    public void fillJoinEntityMeta(AchillesEntityParsingContext context, Map<Class<?>, EntityMeta> entityMetaMap) {
        ...
        for (Entry<PropertyMeta<?, ?>, Class<?>> entry : context.getJoinPropertyMetaToBeFilled().entrySet()) {
            ...
            log.trace("Join property meta built for entity class {} : {}", clazz.getCanonicalName(), propertyMeta);
        }
    }
    ...
    private void processWideMap(AchillesEntityParsingContext context, PropertyMeta<?, ?> idMeta) {
        for (Entry<PropertyMeta<?, ?>, String> entry : context.getWideMaps().entrySet()) {
            ...
            log.debug("Fill wide map meta {} of entity class {}", externalWideMapMeta.getPropertyName(), context
                    .getCurrentEntityClass().getCanonicalName());
    ...
    private void processJoinWideMap(AchillesEntityParsingContext context, String columnFamilyName,
        ...
        for (Entry<PropertyMeta<?, ?>, String> entry : context.getJoinWideMaps().entrySet()) {
            ...
            log.debug("Fill join wide map meta {} of entity class {}", joinExternalWideMapMeta.getPropertyName(),
    ...
    private void completeCounterPropertyMeta(AchillesEntityParsingContext context, PropertyMeta<?, ?> idMeta) {
    ...
    private void saveConsistencyLevel(AchillesEntityParsingContext context, String columnFamilyName,
