...
public class CompoundKeyParser
{
	...
	public CompoundKeyProperties parseCompoundKey(Class<?> keyClass)
	{
		...
		Validator.validateBeanMappingTrue(
				components.size() > 1,
				"There should be at least 2 components for the @CompoundKey class '"
			...
			Annotation[] paramAnnotations, Set<String> uniqueJsonPropertyNames)
	{
		...
		Validator.validateBeanMappingTrue(jsonPropertyName.isPresent(), "The constructor '"
			...
			Annotation[] paramAnnotations)
	{
		...
		Validator.validateBeanMappingTrue(orderO.isPresent(),
				"The constructor '" + constructor.toString()
						+ "' of @CompoundKey class '" + keyClass.getCanonicalName()
						+ "' should have all its params annotated with @Order AND @JsonProperty");
