...
public class CompoundKeyParser {
			...
			if (accept)
				Validator
						.validateBeanMappingTrue(
								isNotBlank(((JsonProperty) annot).value()),
	...
	private Constructor<?> scannConstructorParams(Class<?> keyClass,
			Map<Integer, Field> components) {
		Set<Integer> orders = new HashSet<Integer>();
		int orderSum = 0;
		...
		for (int i = 0; i < paramTypes.length; i++) {
			...
			String propertyName = validateJsonPropertyAnnotation(keyClass,
					constructor, paramAnnotations, uniqueJsonPropertyNames);
			int order = retrieveOrder(keyClass, constructor, paramAnnotations);
			orderSum = validateNoDuplicateOrderAndType(keyClass, orders,
					orderSum, order, paramTypes[i]);
			Field matchingField = findFieldMatchingConstructorParam(keyClass,
					paramTypes[i], propertyName);
	...
	private Constructor<?> findMatchingConstructor(Class<?> keyClass) {
		@SuppressWarnings({ "unchecked", "rawtypes" })
		Set<Constructor> candidateConstructors = ReflectionUtils
				.getConstructors(
						keyClass,
						Predicates
								.and((Predicate) withAnyParameterAnnotation(Order.class),
	...
	private String validateJsonPropertyAnnotation(Class<?> keyClass,
			Constructor<?> constructor, Annotation[] paramAnnotations,
			Set<String> uniqueJsonPropertyNames) {
		...
		Optional<String> jsonPropertyName = FluentIterable
				...
				.transform(retrieveJsonPropertyName).first();
		...
		Validator
				.validateBeanMappingTrue(
						uniqueJsonPropertyNames.add(jsonPropertyName.get()),
	...
	private int retrieveOrder(Class<?> keyClass, Constructor<?> constructor,
		...
		Optional<Integer> orderO = FluentIterable
				.from(Arrays.asList(paramAnnotations))
				.filter(hasOrderAnnotation).transform(retrieveOrder).first();
	...
	private Field findFieldMatchingConstructorParam(Class<?> keyClass,
			Class<?> componentType, String propertyName) {
		@SuppressWarnings("unchecked")
		Set<Field> matchingFields = getFields(keyClass,
				Predicates.and(withType(componentType), withName(propertyName)));

		Validator
				.validateBeanMappingTrue(
						matchingFields.size() == 1,
						"Cannot find field of type '%s' and name '%s' in the @CompoundKey class %s",
						componentType.getCanonicalName(), propertyName,
						keyClass.getCanonicalName());
