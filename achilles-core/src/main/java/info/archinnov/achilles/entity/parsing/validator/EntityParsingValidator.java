...
public class EntityParsingValidator {
    ...
    public void validateWideRows(EntityParsingContext context) {
        ...
        if (context.isWideRow()) {
            log.debug("Validate that there is at least one property meta for the wide row class {}", context
                    .getCurrentEntityClass()
                    .getCanonicalName());
    ...
    public void validateJoinEntityNotWideRow(PropertyMeta<?, ?> propertyMeta, EntityMeta joinEntityMeta) {
        log.debug("Validate that the join entity for the property {} of the entity class {} is not a wide row",
