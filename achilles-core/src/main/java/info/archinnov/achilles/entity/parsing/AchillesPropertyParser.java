...
import static info.archinnov.achilles.helper.AchillesPropertyHelper.*;
...
import info.archinnov.achilles.helper.AchillesPropertyHelper;
...
/**
 * AchillesPropertyParser
 * 
 * @author DuyHai DOAN
 * 
 */
public class AchillesPropertyParser {
    private static final Logger log = LoggerFactory.getLogger(AchillesPropertyFilter.class);

    private AchillesPropertyHelper achillesPropertyHelper = new AchillesPropertyHelper();
    private AchillesEntityIntrospector achillesEntityIntrospector = new AchillesEntityIntrospector();
    private AchillesPropertyParsingValidator validator = new AchillesPropertyParsingValidator();

    public PropertyMeta<?, ?> parse(AchillesPropertyParsingContext context) {
        ...
        context
                .setCustomConsistencyLevels(achillesPropertyHelper.hasConsistencyAnnotation(context.getCurrentField()));
    ...
    protected PropertyMeta<Void, ?> parseSimpleProperty(AchillesPropertyParsingContext context) {
    ...
    protected PropertyMeta<Void, ?> parseCounterProperty(AchillesPropertyParsingContext context) {
    ...
    public <V> PropertyMeta<Void, V> parseListProperty(AchillesPropertyParsingContext context) {
    ...
    public <V> PropertyMeta<Void, V> parseSetProperty(AchillesPropertyParsingContext context) {
    ...
    protected <K, V> PropertyMeta<K, V> parseMapProperty(AchillesPropertyParsingContext context) {
    ...
    protected <K, V> PropertyMeta<K, V> parseWideMapProperty(AchillesPropertyParsingContext context) {
    ...
    public void fillWideMap(AchillesEntityParsingContext context, PropertyMeta<?, ?> idMeta,
    ...
    private void inferPropertyNameAndExternalTableName(AchillesPropertyParsingContext context) {
    ...
    private MultiKeyProperties parseMultiKey(Class<?> keyClass) {
        ...
        if (MultiKey.class.isAssignableFrom(keyClass)) {
            ...
            AchillesPropertyParsingValidator.validateAllowedTypes(keyClass, allowedTypes,
    ...
    private void saveWideMapForDeferredBinding(AchillesPropertyParsingContext context, PropertyMeta<?, ?> propertyMeta) {
    ...
    private void fillWideMapCustomConsistencyLevels(AchillesPropertyParsingContext context,
            PropertyMeta<?, ?> propertyMeta) {
        log.trace("Determining wide map meta {} custom consistency levels", propertyMeta);
        boolean isCustomConsistencyLevel = achillesPropertyHelper.hasConsistencyAnnotation(context.getCurrentField());
        ...
        if (isCustomConsistencyLevel) {
            ...
            context.getConfigurableCLPolicy().setConsistencyLevelForRead(consistencyLevels.left, externalTableName);
            context.getConfigurableCLPolicy().setConsistencyLevelForWrite(consistencyLevels.right, externalTableName);
    ...
    private void parseSimpleCounterConsistencyLevel(AchillesPropertyParsingContext context,
            PropertyMeta<?, ?> propertyMeta) {

        log.trace("Parse custom consistency levels for counter property {}", propertyMeta);
        Pair<ConsistencyLevel, ConsistencyLevel> consistencyLevels = achillesPropertyHelper.findConsistencyLevels(
