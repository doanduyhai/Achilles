...
public class PropertyParsingValidator {
    ...
    public void validateWideRowHasNoExternalWideMap(PropertyParsingContext context) {
        log.debug("Validate that the wide map property {} for wide row {} has no external column family",
        ...
        if (context.isExternal() && context.isColumnFamilyDirectMapping()) {
            throw new AchillesBeanMappingException("Error for field '" + context.getCurrentField().getName()
