...
public class OptionsTest {
	...
	public void should_duplicate_without_ttl_and_timestamp() throws Exception {
		Options options = OptionsBuilder.withConsistency(EACH_QUORUM).ttl(10)
				.timestamp(100L);

		Options newOptions = options.duplicateWithoutTtlAndTimestamp();

		assertThat(newOptions.getConsistencyLevel().get())
				.isSameAs(EACH_QUORUM);
	...
	public void should_duplicate_with_new_consistency_level() throws Exception {
		...
		Options newOptions = options
				.duplicateWithNewConsistencyLevel(LOCAL_QUORUM);

		assertThat(newOptions.getConsistencyLevel().get()).isSameAs(
				LOCAL_QUORUM);
