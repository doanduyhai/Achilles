...
import info.archinnov.achilles.entity.manager.Toto;
...
public class ArgumentExtractorTest {
	...
	@Test
	public void testInitEventInterceptor() throws Exception {
		doCallRealMethod().when(extractor).initEventInterceptor(configMap);

		List<Toto> interceptors = new ArrayList<Toto>();
		interceptors.add(new Toto());
		configMap.put(ConfigurationParameters.EVENT_INTERCEPTORS, interceptors);
		List<EventInterceptor<? extends Object>> a = extractor
				.initEventInterceptor(configMap);
		a.get(0).getClass().getDeclaredMethods()[2].getGenericReturnType()
				.toString();
		// EventInterceptor<? extends Object> e = toto;
		// printType(e.getClass().getDeclaredMethods()[0].getGenericReturnType());
		// e.getClass().getDeclaredMethods()[1].getGenericReturnType().toString();
	}
}
