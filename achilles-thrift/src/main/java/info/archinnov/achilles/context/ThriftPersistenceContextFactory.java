...
public class ThriftPersistenceContextFactory implements PersistenceContextFactory {
	...
	public ThriftPersistenceContext newContext(Object entity, Options options) {
		...
		return new ThriftPersistenceContext(meta, configContext, daoContext, flushContext, entity, options);
	}
	...
	public ThriftPersistenceContext newContext(Class<?> entityClass, Object primaryKey, Options options) {
		...
		return new ThriftPersistenceContext(meta, configContext, daoContext, flushContext, entityClass, primaryKey,
				options);
	}

	@Override
	public ThriftPersistenceContext newContextForSliceQuery(Class<?> entityClass, List<Object> partitionComponents,
			ConsistencyLevel cl) {
		EntityMeta meta = entityMetaMap.get(entityClass);
		Object embeddedId = meta.instanciateEmbeddedIdWithPartitionKey(partitionComponents);

		ThriftImmediateFlushContext flushContext = buildImmediateFlushContext(OptionsBuilder.withConsistency(cl));

		return new ThriftPersistenceContext(meta, configContext, daoContext, flushContext, entityClass, embeddedId,
