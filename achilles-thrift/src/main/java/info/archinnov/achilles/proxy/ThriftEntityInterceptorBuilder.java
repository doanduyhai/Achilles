...
public class ThriftEntityInterceptorBuilder<T> {
	private static final Logger log = LoggerFactory
			.getLogger(ThriftEntityInterceptorBuilder.class);

	private T target;
	private Set<Method> alreadyLoaded = new HashSet<Method>();
	private ThriftPersistenceContext context;

	public static <T> ThriftEntityInterceptorBuilder<T> builder(
			ThriftPersistenceContext context, T entity) {
		return new ThriftEntityInterceptorBuilder<T>(context, entity);
	}

	public ThriftEntityInterceptorBuilder(ThriftPersistenceContext context,
			T entity) {
	...
	public ThriftEntityInterceptor<T> build() {
		...
		if (entityMeta.isClusteredEntity()) {
			Validator.validateNotNull(context.getWideRowDao(),
					"Column Family Dao for '%s' should not be null", className);
		} else {
			Validator.validateNotNull(context.getEntityDao(),
					"Entity dao for '%s' should not be null", className);
		}
	...
	public ThriftEntityInterceptorBuilder<T> alreadyLoaded(
