...
public abstract class ThriftAbstractSliceIterator<HCOLUMN> implements
		Iterator<HCOLUMN> {
	private static final Logger log = LoggerFactory
			.getLogger(ThriftAbstractSliceIterator.class);
	...
	protected ThriftAbstractSliceIterator(
			AchillesConsistencyLevelPolicy policy, String columnFamily,
		...
		this.count = count;
		this.readConsistencyLevelAtInitialization = policy
				.getCurrentReadLevel();
	}
	...
	public boolean hasNext() {
		if (iterator == null) {
			...
			log.trace("Reload another batch of {} elements for {}", count,
					type());
	...
	protected <T> T executeWithInitialConsistencyLevel(
		...
		if (readConsistencyLevelAtInitialization != null) {
			log.trace("Save current consistency level {} by {}",
					policy.getCurrentReadLevel(), type());
			...
			try {
				...
				log.trace("Restore current consistency level {} by {}",
						currentReadLevel, type());
				...
				log.trace(
						...
						count, readConsistencyLevelAtInitialization.name(),
						type());
