...
public class ThriftClusteredEntityIterator<T> extends
		ThriftAbstractClusteredEntityIterator<T> {
	private static final Logger log = LoggerFactory
			.getLogger(ThriftClusteredEntityIterator.class);

	private ThriftAbstractSliceIterator<HColumn<Composite, Object>> sliceIterator;

	public ThriftClusteredEntityIterator(
			Class<T> entityClass,
	...
	public T next() {
		...
		if (context.isValueless()) {
			target = transformer.buildClusteredEntityWithIdOnly(entityClass,
					context, hColumn.getName().getComponents());
		} else {
			target = transformer.buildClusteredEntity(entityClass, context,
					hColumn);
		}
