...
public class ThriftColumnFamilyValidator {
    ...
    public void validateCFForEntity(ColumnFamilyDefinition cfDef, EntityMeta entityMeta) {
        ...
        if (!StringUtils.equals(cfDef.getKeyValidationClass(), idSerializer.getComparatorType().getClassName())) {
            throw new AchillesInvalidColumnFamilyException("The column family '" + entityMeta.getTableName()
        ...
        if (!StringUtils.equals(comparatorType, ENTITY_COMPARATOR_TYPE_CHECK)) {
            throw new AchillesInvalidColumnFamilyException("The column family '" + entityMeta.getTableName()
                    + "' comparator type '" + comparatorType + "' should be '" + ENTITY_COMPARATOR_TYPE_CHECK + "'");
        }
    }
    ...
    public void validateWideRowForProperty(ColumnFamilyDefinition cfDef, PropertyMeta<?, ?> propertyMeta,
        ...
        if (!StringUtils.equals(comparatorType, comparatorTypeAlias)) {
            throw new AchillesInvalidColumnFamilyException("The column family '" + tableName
                    + "' comparator type should be '" + comparatorTypeAlias + "' : " + comparatorType);
        }
    }
    ...
    public void validateWideRowForClusteredEntity(ColumnFamilyDefinition cfDef, EntityMeta meta, String tableName) {
        ...
        if (!StringUtils.equals(cfDef.getKeyValidationClass(), keyValidationType)) {
            throw new AchillesInvalidColumnFamilyException("The column family '" + tableName
                    + "' key validation type should be '" + keyValidationType + "'");
        }
        ...
        if (!StringUtils.equals(cfDef.getDefaultValidationClass(), valueValidationType)) {
            throw new AchillesInvalidColumnFamilyException("The column family '" + tableName
                    + "' default validation type should be '" + valueValidationType + "'");
        }
    }
    ...
    public void validateCounterCF(ColumnFamilyDefinition cfDef) {
        ...
        if (!StringUtils.equals(keyValidation, COUNTER_KEY_CHECK)) {
            throw new AchillesInvalidColumnFamilyException("The column family '" + AchillesCounter.THRIFT_COUNTER_CF
