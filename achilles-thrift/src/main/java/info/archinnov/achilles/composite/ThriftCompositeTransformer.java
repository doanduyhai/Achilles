...
public class ThriftCompositeTransformer
{
    ...
    public <K> K buildCounterKey(PropertyMeta<K, ?> pm, HCounterColumn<Composite> hColumn)
    {
        K key;
        Serializer<K> keySerializer = ThriftSerializerTypeInferer.getSerializer(pm
                .getKeyClass());
        if (pm.isCompound())
        {
            key = compoundKeyMapper.fromCompositeToCompound(pm, hColumn
                    .getName().getComponents());
        }
        else
        {
