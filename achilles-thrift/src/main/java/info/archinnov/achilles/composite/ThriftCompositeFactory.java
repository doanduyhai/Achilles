...
public class ThriftCompositeFactory {
	...
	public <K, V, T> Composite createCompositeForClustered(
			PropertyMeta propertyMeta, T key) {
		log.trace("Creating base composite for propertyMeta {}",
				propertyMeta.getPropertyName());
		return compoundKeyMapper.fromCompoundToCompositeForInsertOrGet(key,
				propertyMeta);
	}

	public Composite[] createForClusteredQuery(PropertyMeta idMeta,
			List<Object> clusteringFrom, List<Object> clusteringTo,
			BoundingMode bounding, OrderingMode ordering) {

		compoundKeyValidator.validateComponentsForSliceQuery(idMeta,
				clusteringFrom, clusteringTo, ordering);
		ComponentEquality[] equalities = calculator.determineEquality(bounding,
				ordering);

		final Composite from = compoundKeyMapper
				.fromComponentsToCompositeForQuery(clusteringFrom, idMeta,
						equalities[0]);
		final Composite to = compoundKeyMapper
				.fromComponentsToCompositeForQuery(clusteringTo, idMeta,
						equalities[1]);
	...
	public Composite createKeyForCounter(String fqcn, Object key,
			PropertyMeta idMeta) {
		log.trace(
				"Creating composite counter row key for entity class {} and primary key {}",
				fqcn, key);
	...
	public <K, V> Composite createBaseForGet(PropertyMeta propertyMeta) {
		log.trace("Creating base composite for propertyMeta {} get",
				propertyMeta.getPropertyName());

		Composite composite = new Composite();
		composite.addComponent(0, propertyMeta.type().flag(),
	...
	public Composite createBaseForClusteredGet(Object compoundKey,
			PropertyMeta idMeta) {
		return compoundKeyMapper.fromCompoundToCompositeForInsertOrGet(
				compoundKey, idMeta);
	}
	...
	public <K, V> Composite createBaseForCounterGet(PropertyMeta propertyMeta) {
		...
		Composite composite = new Composite();
		composite.addComponent(0, propertyMeta.getPropertyName(),
				ComponentEquality.EQUAL);
		return composite;
	}
	...
	public <K, V> Composite createBaseForQuery(PropertyMeta propertyMeta,
		...
		log.trace(
				"Creating base composite for propertyMeta {} query and equality {}",
	...
	public <K, V> Composite createForBatchInsertSingleValue(
			PropertyMeta propertyMeta) {
		log.trace(
				"Creating base composite for propertyMeta {} for single value batch insert",
				propertyMeta.getPropertyName());
	...
	public <K, V> Composite createForBatchInsertSingleCounter(
			PropertyMeta propertyMeta) {
		log.trace(
		...
		Composite composite = new Composite();
		composite.setComponent(0, propertyMeta.getPropertyName(), STRING_SRZ,
				STRING_SRZ.getComparatorType().getTypeName());
		return composite;
	}
