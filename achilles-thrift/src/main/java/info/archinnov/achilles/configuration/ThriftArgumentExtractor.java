...
public class ThriftArgumentExtractor extends ArgumentExtractor {
	...
	public Cluster initCluster(Map<String, Object> configurationMap) {
		...
		if (cluster == null) {
			String cassandraHost = (String) configurationMap
					.get(HOSTNAME_PARAM);
			String cassandraClusterName = (String) configurationMap
					.get(CLUSTER_NAME_PARAM);
	...
	public Keyspace initKeyspace(Cluster cluster,
			ThriftConsistencyLevelPolicy consistencyPolicy,
			Map<String, Object> configurationMap) {
		Keyspace keyspace = (Keyspace) configurationMap.get(KEYSPACE_PARAM);
		if (keyspace == null) {
			String keyspaceName = (String) configurationMap
