...
public abstract class ThriftAbstractDao {
	...
	public <K, V> void insertColumnBatch(K key, Composite name, V value,
			Optional<Integer> ttlO, Optional<Long> timestampO,
			Mutator<K> mutator) {
		if (log.isTraceEnabled())
			log.trace("Insert column {} into column family {} with key {}",
					format(name), columnFamily, key);

		HColumn<Composite, V> column;
		if (ttlO.isPresent() && timestampO.isPresent()) {
			column = HFactory.createColumn(name, value, timestampO.get(),
					ttlO.get(), columnNameSerializer, this.<V> valSrz());
		} else if (ttlO.isPresent()) {
			column = HFactory.createColumn(name, value, ttlO.get(),
					columnNameSerializer, this.<V> valSrz());
		} else if (timestampO.isPresent()) {
			column = HFactory.createColumn(name, value, timestampO.get(),
					columnNameSerializer, this.<V> valSrz());
		} else {
			column = HFactory.createColumn(name, value, columnNameSerializer,
					this.<V> valSrz());
		}
	...
	public <K, V> V getValue(final K key, final Composite name) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Get value from column family {} with key {} and column name {}",
	...
	public <K, V> HColumn<Composite, V> getColumn(final K key,
			final Composite name) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Get column from column family {} with key {} and column name {}",
					columnFamily, key, format(name));
		}
	...
	public <K, V> void setValue(K key, Composite name, V value) {
		log.trace(
				"Set value {} to column family {} with key {} , column name {}",
				value, columnFamily, key, name);

		Mutator<K> mutator = HFactory
				.createMutator(keyspace, this.<K> rowSrz());
		this.setValueBatch(key, name, value, Optional.<Integer> absent(),
				Optional.<Long> absent(), mutator);
		this.executeMutator(mutator);
	}

	public <K, V> void setValueBatch(K key, Composite name, V value,
			Optional<Integer> ttlO, Optional<Long> timestampO,
			Mutator<K> mutator) {
		if (log.isTraceEnabled()) {
			log.trace(
	...
	public <K> void removeColumnBatch(K key, Composite name, Mutator<K> mutator) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Remove column name {} as batch mutation from column family {} with key {} ",
					format(name), columnFamily, key);
		}
		mutator.addDeletion(key, columnFamily, name, columnNameSerializer);
	}

	public <K, V> void removeColumnRangeBatch(K key, Composite start,
			Composite end, Mutator<K> mutator) {
		if (log.isTraceEnabled()) {
			log.trace(
		...
		this.removeColumnRangeBatch(key, start, end, false, Integer.MAX_VALUE,
				mutator);
	}

	public <K, V> void removeColumnRangeBatch(K key, Composite start,
			Composite end, boolean reverse, int count, Mutator<K> mutator) {
		if (log.isTraceEnabled()) {
			log.trace(
					...
					count, format(start), format(end), columnFamily, key,
					reverse);
		}
		List<HColumn<Composite, V>> columns = createSliceQuery(keyspace,
				this.<K> rowSrz(), columnNameSerializer, this.<V> valSrz())
				.setColumnFamily(columnFamily).setKey(key)
				.setRange(start, end, reverse, count).execute().get()
				.getColumns();

		for (HColumn<Composite, V> column : columns) {
			mutator.addDeletion(key, columnFamily, column.getName(),
					columnNameSerializer);
		}
	}

	public <K, V> List<V> findValuesRange(final K key, final Composite start,
		...
		List<HColumn<Composite, V>> columns = reinitConsistencyLevels(new SafeExecutionContext<List<HColumn<Composite, V>>>() {
			...
			public List<HColumn<Composite, V>> execute() {
				return createSliceQuery(keyspace,
						ThriftAbstractDao.this.<K> rowSrz(),
						columnNameSerializer,
						ThriftAbstractDao.this.<V> valSrz())
						.setColumnFamily(columnFamily).setKey(key)
	...
	public <K, V> List<Pair<Composite, V>> findColumnsRange(final K key,
			final Composite start, final Composite end, final boolean reverse,
			final int count) {
		if (log.isTraceEnabled()) {
			log.trace(
	...
	public <K, V> List<HColumn<Composite, V>> findRawColumnsRange(final K key,
			final Composite start, final Composite end, final int count,
		...
		return reinitConsistencyLevels(new SafeExecutionContext<List<HColumn<Composite, V>>>() {
			@Override
			public List<HColumn<Composite, V>> execute() {
				List<HColumn<Composite, V>> columns = createSliceQuery(
						keyspace, ThriftAbstractDao.this.<K> rowSrz(),
						columnNameSerializer,
						ThriftAbstractDao.this.<V> valSrz())
						.setColumnFamily(columnFamily).setKey(key)
						.setRange(start, end, reverse, count).execute().get()
	...
	public <K, V> List<HCounterColumn<Composite>> findCounterColumnsRange(
			final K key, final Composite start, final Composite end,
		...
		return reinitConsistencyLevels(new SafeExecutionContext<List<HCounterColumn<Composite>>>() {
			...
			public List<HCounterColumn<Composite>> execute() {
				return HFactory
						.createCounterSliceQuery(keyspace,
								ThriftAbstractDao.this.<K> rowSrz(),
								columnNameSerializer)
						.setColumnFamily(columnFamily).setKey(key)
						.setRange(start, end, reverse, count).execute().get()
	...
	public <K, V> ThriftSliceIterator<K, V> getColumnsIterator(K key,
			Composite start, Composite end, boolean reverse, int length) {
		if (log.isTraceEnabled()) {
			log.trace(
					...
					format(start), format(end), columnFamily, key, reverse,
					length);
		}

		SliceQuery<K, Composite, V> query = createSliceQuery(keyspace,
				ThriftAbstractDao.this.<K> rowSrz(), columnNameSerializer,
				ThriftAbstractDao.this.<V> valSrz()).setColumnFamily(
				columnFamily).setKey(key);

		return new ThriftSliceIterator<K, V>(policy, columnFamily, query,
				start, end, reverse, length);
	}

	public <K, V> ThriftCounterSliceIterator<K> getCounterColumnsIterator(
			K key, Composite start, Composite end, boolean reverse, int length) {
		if (log.isTraceEnabled()) {
			log.trace(
		...
		this.policy.loadConsistencyLevelForRead(columnFamily);
		SliceCounterQuery<K, Composite> query = createCounterSliceQuery(
	...
	public <K, V> Rows<K, Composite, V> multiGetSliceRange(final List<K> keys,
		...
		if (log.isTraceEnabled()) {
			log.trace(
					...
					format(start), format(end), columnFamily,
					StringUtils.join(keys, ","), reverse, size);
		}
	...
	public <K> void removeRowBatch(K key, Mutator<K> mutator) {
		log.trace(
				"Remove row as batch mutation from column family {} with key {}",
				columnFamily, key);
	...
	public <K> void incrementCounter(K key, Composite name, Long value) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Incrementing counter column {} with key {} from column family {} by {}",
					format(name), key, columnFamily, value);
		}
		Mutator<K> mutator = buildMutator();
		mutator.addCounter(key, columnFamily,
				new HCounterColumnImpl<Composite>(name, value,
	...
	public <K> void decrementCounter(K key, Composite name, Long value) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Decrementing counter column {} with key {} from column family {} by {}",
					format(name), key, columnFamily, value);
		}
		Mutator<K> mutator = buildMutator();
		mutator.addCounter(key, columnFamily,
				new HCounterColumnImpl<Composite>(name, value * -1L,
						ThriftSerializerUtils.COMPOSITE_SRZ));
		executeMutator(mutator);
	}
	...
	public <K> Long getCounterValue(K key, Composite name) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Get counter value column {} with key {} from column family {}",
	...
	public <K> HCounterColumn<Composite> getCounterColumn(K key, Composite name) {
		if (log.isTraceEnabled()) {
			log.trace(
					"Get counter  column {} with key {} from column family {}",
					format(name), key, columnFamily);
		}

		final CounterQuery<K, Composite> counter = new ThriftCounterColumnQuery<K, Composite>(
				keyspace, this.<K> rowSrz(), columnNameSerializer)
				.setColumnFamily(columnFamily).setKey(key).setName(name);
	...
	public <K> void removeCounterRowBatch(K key, Mutator<K> mutator) {
		log.trace(
				"Remove counter row as batch mutation with key {} from column family {}",
				key, columnFamily);

		SliceCounterQuery<K, Composite> query = HFactory
				.createCounterSliceQuery(keyspace, this.<K> rowSrz(),
						columnNameSerializer).setColumnFamily(columnFamily)
				.setKey(key);

		ThriftCounterSliceIterator<K> iterator = new ThriftCounterSliceIterator<K>(
				policy, columnFamily, query, (Composite) null,
				(Composite) null, false, DEFAULT_LENGTH);

		while (iterator.hasNext()) {
			HCounterColumn<Composite> counterCol = iterator.next();
			mutator.deleteCounter(key, columnFamily, counterCol.getName(),
					columnNameSerializer);
		}
	}
	...
	public void truncateCounters() {
		cluster.truncate(keyspace.getKeyspaceName(),
	...
	public <K> void executeMutator(final Mutator<K> mutator) {
		log.trace(
				"Execute safely mutator with {} mutations for column family {}",
	...
	protected <T> Serializer<T> rowSrz() {
		return ThriftSerializerTypeInferer
				.<T> getSerializer((Class<?>) rowkeyAndValueClasses.left);
	}
