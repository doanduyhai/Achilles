...
public class ThriftQueryExecutorTest
{
	@InjectMocks
	private ThriftQueryExecutor executor;
	...
	{
		Whitebox.setInternalState(executor, "factory", factory);
		Whitebox.setInternalState(executor, "proxifier", proxifier);
		Whitebox.setInternalState(executor, "invoker", invoker);
		Whitebox.setInternalState(executor, "executorImpl", executorImpl);
		...
		when(invoker.instanciateEmbeddedIdWithPartitionKey(idMeta, partitionKey)).thenReturn(
				compoundKey);
