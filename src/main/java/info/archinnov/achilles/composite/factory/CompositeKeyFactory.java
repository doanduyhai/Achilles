...
public class CompositeKeyFactory {
    ...
    public <K, V, T> Composite createBaseComposite(PropertyMeta<K, V> propertyMeta, T keyValue) {
        ...
        if (propertyMeta.isSingleKey()) {
            ...
            composite.setComponent(0, keyValue, keySerializer, keySerializer.getComparatorType().getTypeName());
        } else {
            ...
            for (Object value : keyValues) {
                Validator.validateNotNull(value, "The values for the for the key of WideMap '" + propertyName
                        + "' should not be null");
            }
    ...
    @SuppressWarnings("unchecked")
    public <K, V, T> Composite createForQuery(PropertyMeta<K, V> propertyMeta, T keyValue, ComponentEquality equality) {
