...
import info.archinnov.achilles.proxy.wrapper.builder.AchillesIteratorWrapperBuilder;
...
/**
 * AchillesCollectionWrapper
 * 
 * @author DuyHai DOAN
 * 
 */
public class AchillesCollectionWrapper<V> extends AchillesAbstractWrapper<Void, V> implements
		Collection<V>
{
	private static final Logger log = LoggerFactory.getLogger(AchillesCollectionWrapper.class);

	protected Collection<V> target;

	public AchillesCollectionWrapper(Collection<V> target) {
		this.target = target;
	}
	...
	public Iterator<V> iterator()
	{
		...
		return AchillesIteratorWrapperBuilder.builder(context, this.target.iterator()) //
				.dirtyMap(dirtyMap)
				//
				.setter(setter)
				//
