...
public abstract class AbstractDao<K, N extends AbstractComposite, V> {
    ...
    public void removeColumnRangeBatch(K key, N start, N end, boolean reverse, int count, Mutator<K> mutator) {
        List<HColumn<N, V>> columns = createSliceQuery(keyspace, keySerializer, columnNameSerializer, valueSerializer)
                .setColumnFamily(columnFamily).setKey(key).setRange(start, end, reverse, count).execute().get()
                .getColumns();
    ...
    public AchillesSliceIterator<K, N, V> getColumnsIterator(K key, N startName, boolean reverse, int length) {
        return getColumnsIterator(key, startName, null, reverse, length);
    }

    public AchillesSliceIterator<K, N, V> getColumnsIterator(K key, N startName, N endName, boolean reverse) {
    ...
    public <KEY, VALUE> AchillesJoinSliceIterator<K, N, V, KEY, VALUE> getJoinColumnsIterator(
            PropertyMeta<KEY, VALUE> propertyMeta, K key, N startName, N endName, boolean reversed, int count) {
        SliceQuery<K, N, V> query = createSliceQuery(keyspace, keySerializer, columnNameSerializer, valueSerializer)
                .setColumnFamily(columnFamily).setKey(key);

        return new AchillesJoinSliceIterator<K, N, V, KEY, VALUE>(propertyMeta, query, startName, endName, reversed,
                count);
    }

    public CounterColumnSliceIterator<K, N> getCounterColumnsIterator(K key, N startName, boolean reverse, int length) {
        SliceCounterQuery<K, N> query = createCounterSliceQuery(keyspace, keySerializer, columnNameSerializer)
                .setColumnFamily(columnFamily).setKey(key);
    ...
    public <KEY, NAME extends AbstractComposite, VALUE> AchillesSliceIterator<KEY, NAME, VALUE> getSpecificColumnsIterator(
            Serializer<KEY> keySz, Serializer<NAME> nameSz, Serializer<VALUE> valueSz, String CF, KEY key,
            NAME startName, boolean reverse, int length) {
        SliceQuery<KEY, NAME, VALUE> query = createSliceQuery(keyspace, keySz, nameSz, valueSz).setColumnFamily(CF)
                .setKey(key);

        return new AchillesSliceIterator<KEY, NAME, VALUE>(query, startName, (NAME) null, reverse, length);
    }
    ...
    public Rows<K, N, V> multiGetSliceRange(List<K> keys, N startName, N endName, boolean reverse, int size) {
        return HFactory.createMultigetSliceQuery(keyspace, keySerializer, columnNameSerializer, valueSerializer)
                .setColumnFamily(columnFamily).setKeys(keys).setRange(startName, endName, reverse, size).execute()
                .get();
    }
    ...
    public void insertCounter(K key, N name, Long value, Mutator<K> mutator) {
        ...
        mutator.insertCounter(key, columnFamily, HFactory.createCounterColumn(name, value, columnNameSerializer));
    }
