...
public class KeyValueFactory {
    ...
    public <K, V> List<V> createValueListForDynamicComposite(PropertyMeta<K, V> propertyMeta,
        ...
        return Lists.transform(hColumns, dynamicCompositeTransformer.buildValueTransformer(propertyMeta));
    }
    ...
    public <K, V> List<K> createKeyListForDynamicComposite(PropertyMeta<K, V> propertyMeta,
            List<HColumn<DynamicComposite, String>> hColumns) {
        return Lists.transform(hColumns, dynamicCompositeTransformer.buildKeyTransformer(propertyMeta));
    }

    public <K, V> List<KeyValue<K, V>> createKeyValueListForDynamicComposite(PropertyMeta<K, V> propertyMeta,
            List<HColumn<DynamicComposite, String>> hColumns) {
        return Lists.transform(hColumns, dynamicCompositeTransformer.buildKeyValueTransformer(propertyMeta));
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    public <K, V> List<KeyValue<K, V>> createJoinKeyValueListForDynamicComposite(PropertyMeta<K, V> propertyMeta,
            List<HColumn<DynamicComposite, String>> hColumns) {
        List<K> keys = Lists.transform(hColumns, dynamicCompositeTransformer.buildKeyTransformer(propertyMeta));
        List<Object> joinIds = Lists.transform(hColumns,
                dynamicCompositeTransformer.buildRawValueTransformer(propertyMeta));

        Map<Object, V> joinEntities = joinHelper.loadJoinEntities(propertyMeta.getValueClass(), joinIds,
                (EntityMeta) propertyMeta.getJoinProperties().getEntityMeta());
        List<Integer> ttls = Lists.transform(hColumns, dynamicCompositeTransformer.buildTtlTransformer());
    ...
    public <K, V> List<V> createJoinValueListForComposite(PropertyMeta<K, V> propertyMeta,
            List<HColumn<Composite, ?>> hColumns) {
        List<?> joinIds = Lists.transform(hColumns, compositeTransformer.buildRawValueTransformer());
        Map<?, V> joinEntities = joinHelper.loadJoinEntities(propertyMeta.getValueClass(), joinIds,
                (EntityMeta) propertyMeta.getJoinProperties().getEntityMeta());
        List<V> result = new ArrayList<V>();
        for (Object joinId : joinIds) {
            V joinEntity = joinEntities.get(joinId);
            V proxy = helper.buildProxy(joinEntity, propertyMeta.getJoinProperties().getEntityMeta());
            result.add(proxy);
        }
    ...
    public <K, V> List<KeyValue<K, V>> createKeyValueListForComposite(PropertyMeta<K, V> propertyMeta,
            List<HColumn<Composite, ?>> hColumns) {
        return Lists.transform(hColumns, compositeTransformer.buildKeyValueTransformer(propertyMeta));
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    public <K, V> List<KeyValue<K, V>> createJoinKeyValueListForComposite(PropertyMeta<K, V> propertyMeta,
            List<HColumn<Composite, ?>> hColumns) {
        List<K> keys = Lists.transform(hColumns, compositeTransformer.buildKeyTransformer(propertyMeta));
        List<Object> joinIds = Lists.transform(hColumns, compositeTransformer.buildRawValueTransformer());
        Map<Object, V> joinEntities = joinHelper.loadJoinEntities(propertyMeta.getValueClass(), joinIds,
                (EntityMeta) propertyMeta.getJoinProperties().getEntityMeta());
    ...
    // Counter
    public <K> KeyValue<K, Long> createCounterKeyValueForDynamicComposite(PropertyMeta<K, Long> propertyMeta,
            HColumn<DynamicComposite, Long> hColumn) {
        //PropertyMeta<K, Long>
        // HColumn<DynamicComposite, Long> column
        return dynamicCompositeTransformer.buildCounterKeyValueFromDynamicComposite(propertyMeta, hColumn);
    }
    ...
    public <K> List<KeyValue<K, Long>> createCounterKeyValueListForDynamicComposite(
        ...
        return Lists.transform(hColumns, dynamicCompositeTransformer.buildCounterKeyValueTransformer(propertyMeta));
    }

    public <K> List<Long> createCounterValueListForDynamicComposite(PropertyMeta<K, Long> propertyMeta,
            List<HColumn<DynamicComposite, Long>> hColumns) {
        return Lists.transform(hColumns, dynamicCompositeTransformer.buildCounterValueTransformer(propertyMeta));
    }
