    ...
    public EntityMeta build() {
        ...
        if (columnFamilyDirectMapping) {
            ...
            if (isSupportedType(wideMapMeta.getValueClass())) {
                dao = new GenericCompositeDao(keyspace, idSerializer, valueSerializer, this.columnFamilyName);
            } else {
                dao = new GenericCompositeDao(keyspace, idSerializer, STRING_SRZ, this.columnFamilyName);
            }

            meta.setColumnFamilyDao(dao);
        } else {
            meta.setEntityDao(new GenericDynamicCompositeDao(keyspace, idSerializer, this.columnFamilyName));
        }
        return meta;
    }
