...
public class ThriftEntityManager implements EntityManager {
    ...
    public <T> T find(Class<T> entityClass, Object primaryKey) {
        ...
        EntityMeta<Serializable> entityMeta = (EntityMeta<Serializable>) this.entityMetaMap.get(entityClass);
    ...
    public <ID, T> void startBatch(T entity) {
        ...
        for (PropertyMeta<?, ?> propertyMeta : entityMeta.getPropertyMetas().values()) {
            if (propertyMeta.isJoin()) {
                mutatorMap.put(propertyMeta.getPropertyName(), propertyMeta.joinMeta().getEntityDao().buildMutator());
            }

            if (propertyMeta.type() == EXTERNAL_WIDE_MAP_COUNTER) {
                mutatorMap.put(propertyMeta.getPropertyName(), propertyMeta.counterDao().buildMutator());
            }
        }
        ...
        JpaEntityInterceptor<ID, T> interceptor = (JpaEntityInterceptor<ID, T>) proxy.getCallback(0);
        ...
        for (PropertyMeta<?, ?> propertyMeta : entityMeta.getPropertyMetas().values()) {
            ...
            if (type.isLazy() && !type.isWideMap()) {
                try {
                    ...
                    log.error("Cannot initialize property '" + propertyMeta.getPropertyName() + "' for entity '"
                            + realObject + "'", e);
                }
            }
        }
