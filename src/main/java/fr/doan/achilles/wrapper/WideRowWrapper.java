...
public class WideRowWrapper<ID, K, V> implements WideMap<K, V> {
    ...
    @Override
    public List<KeyValue<K, V>> findRange(K start, K end, boolean inclusiveBounds, boolean reverse, int count) {

        return findRange(start, inclusiveBounds, end, inclusiveBounds, reverse, count);
    }

    @Override
    public List<KeyValue<K, V>> findRange(K start, boolean inclusiveStart, K end, boolean inclusiveEnd,
            boolean reverse, int count) {
        helper.checkBounds(start, end, reverse);

        Composite[] composites = compositeKeyFactory
                .createForQuery(start, inclusiveStart, end, inclusiveEnd, reverse);

        List<HColumn<Composite, Object>> hColumns = dao.findRawColumnsRange(id, composites[0], composites[1],
                reverse, count);
    ...
    @Override
    public KeyValueIterator<K, V> iterator(K start, K end, boolean inclusiveBounds, boolean reverse, int count) {
        return iterator(start, inclusiveBounds, end, inclusiveBounds, reverse, count);
    }

    @Override
    public KeyValueIterator<K, V> iterator(K start, boolean inclusiveStart, K end, boolean inclusiveEnd,
            boolean reverse, int count) {

        Composite[] composites = compositeKeyFactory
                .createForQuery(start, inclusiveStart, end, inclusiveEnd, reverse);

        ColumnSliceIterator<ID, Composite, Object> columnSliceIterator = dao.getColumnsIterator(id, composites[0],
    ...
    public void removeRange(K start, boolean inclusiveStart, K end, boolean inclusiveEnd) {
        ...
        Composite[] composites = compositeKeyFactory.createForQuery(start, inclusiveStart, end, inclusiveEnd, false);
