	...
	public void validateColumnFamilies(Map<Class<?>, EntityMeta<?>> entityMetaMap, boolean forceColumnFamilyCreation)
	{
		for (Entry<Class<?>, EntityMeta<?>> entry : entityMetaMap.entrySet())
		{
			...
			if (cfDef == null)
			{
				...
				{
					throw new InvalidColumnFamilyException("The required column family '" + entry.getValue().getColumnFamilyName()
							+ "' does not exist for entity '" + entry.getValue().getCanonicalClassName() + "'");
				}
			}
			else
			{
