...
public class PropertyParser
{
			...
			String propertyName, Class<?> fieldType)
	{
		...
		if (filter.hasAnnotation(field, JoinColumn.class))
		{
			...
			if (filter.hasAnnotation(field, ManyToMany.class))
			{
		...
		{
			return factory(keyClass, valueClass).type(type).propertyName(propertyName)
					.accessors(accessors).singleKey(true).joinProperties(joinProperties).build();

		}
		else
		{
			return factory(keyClass, valueClass).type(type).propertyName(propertyName)
					.accessors(accessors).singleKey(false) //
