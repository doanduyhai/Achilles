...
public class EntityParser {
    ...
    public EntityMeta<?> parseEntity(Keyspace keyspace, Class<?> entityClass,
        ...
        for (Field field : inheritedFields) {
            if (filter.hasAnnotation(field, Id.class)) {
                ...
                String propertyName = StringUtils.isNotBlank(joinColumn.name()) ? joinColumn.name() : field.getName();
                PropertyMeta<?, ?> joinPropertyMeta = parser.parse(entityClass, field, propertyName);
    ...
    private void validatePropertyMetas(Class<?> entityClass, Map<String, PropertyMeta<?, ?>> propertyMetas) {
    ...
    private void validateWideRow(Class<?> entityClass, boolean wideRow, Map<String, PropertyMeta<?, ?>> propertyMetas) {
        if (wideRow) {
            Validator.validateSize(propertyMetas, 1, "The WideRow entity '" + entityClass.getCanonicalName()
                    + "' should not have more than one property annotated with @Column");

            PropertyType type = propertyMetas.entrySet().iterator().next().getValue().propertyType();

            Validator.validateTrue(type == PropertyType.WIDE_MAP,
