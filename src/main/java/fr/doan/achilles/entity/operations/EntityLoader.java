...
public class EntityLoader {
    ...
    public <T, ID> T load(Class<T> entityClass, ID key, EntityMeta<ID> entityMeta) {
        ...
        try {
            ...
            if (entityMeta.isWideRow()) {
                ...
                List<Pair<DynamicComposite, Object>> columns = entityMeta.getEntityDao().eagerFetchEntity(key);
                if (columns.size() > 0) {
                    entity = entityClass.newInstance();
    ...
    protected <ID, V> V loadSimpleProperty(ID key, GenericEntityDao<ID> dao, PropertyMeta<?, V> propertyMeta) {
    ...
    protected <ID, V> List<V> loadListProperty(ID key, GenericEntityDao<ID> dao, ListMeta<V> listPropertyMeta) {
        ...
        List<Pair<DynamicComposite, Object>> columns = dao
    ...
    protected <ID, V> Set<V> loadSetProperty(ID key, GenericEntityDao<ID> dao, SetMeta<V> setPropertyMeta) {
    ...
    protected <ID, K, V> Map<K, V> loadMapProperty(ID key, GenericEntityDao<ID> dao, MapMeta<K, V> mapPropertyMeta) {
    ...
    public <JOIN_ID, V> V loadJoinEntity(Class<V> entityClass, JOIN_ID joinId, EntityMeta<JOIN_ID> joinEntityMeta) {
        V joinEntity = this.load(entityClass, joinId, joinEntityMeta);

        Validator.validateNotNull(joinEntity, "The join entity '" + entityClass.getCanonicalName() + "' with id '"
                + joinId + "' cannot be found");

        return this.interceptorBuilder.build(joinEntity, joinEntityMeta);
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    protected <ID, V> V loadJoinColumn(ID key, GenericEntityDao<ID> dao, PropertyMeta<?, V> joinPropertyMeta) {
