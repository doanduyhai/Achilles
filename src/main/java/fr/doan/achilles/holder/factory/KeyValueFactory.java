...
public class KeyValueFactory {
    ...
    public <K, V> KeyValue<K, V> createFromDynamicCompositeColumn(HColumn<DynamicComposite, Object> hColumn,
    ...
    public <K, V> List<KeyValue<K, V>> createFromColumnList(List<HColumn<Composite, Object>> hColumns,
            PropertyMeta<K, V> wideMapMeta) {
        List<KeyValue<K, V>> result = new ArrayList<KeyValue<K, V>>();
        if (hColumns != null && hColumns.size() > 0) {
            for (HColumn<Composite, Object> hColumn : hColumns) {
                V value = wideMapMeta.getValue(hColumn.getValue());
                K key = wideMapMeta.getKey(hColumn.getName().get(0, wideMapMeta.getKeySerializer()));
