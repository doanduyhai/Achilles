...
public class EntityLoaderTest
{
	...
	public void setUp()
	{
		...
		Whitebox.setInternalState(loader, "introspector", introspector);
	}
	...
	public void should_load_entity() throws Exception
	{
		...
		doNothing().when(introspector).setValueToField(any(CompleteBean.class), eq(idSetter),
				idCaptor.capture());
	...
	public void should_load_simple_property() throws Exception
	{
		...
		when(dynamicCompositeKeyFactory.createBaseForQuery(propertyMeta, EQUAL)).thenReturn(
				composite);
	...
	public void should_load_list_property() throws Exception
	{
		...
		when(dynamicCompositeKeyFactory.createBaseForQuery(listMeta, EQUAL)).thenReturn(start);
		when(dynamicCompositeKeyFactory.createBaseForQuery(listMeta, GREATER_THAN_EQUAL))
				.thenReturn(end);
	...
	public void should_load_set_property() throws Exception
	{
		...
		when(dynamicCompositeKeyFactory.createBaseForQuery(setMeta, EQUAL)).thenReturn(start);
		when(dynamicCompositeKeyFactory.createBaseForQuery(setMeta, GREATER_THAN_EQUAL))
				.thenReturn(end);
	...
	public void should_load_map_property() throws Exception
	{
		...
		when(dynamicCompositeKeyFactory.createBaseForQuery(mapMeta, EQUAL)).thenReturn(start);
		when(dynamicCompositeKeyFactory.createBaseForQuery(mapMeta, GREATER_THAN_EQUAL))
				.thenReturn(end);
	...
	@Test
	public void should_load_simple_counter_into_object() throws Exception
	{
		BeanWithSimpleCounter bean = new BeanWithSimpleCounter();
		Method setter = BeanWithSimpleCounter.class.getDeclaredMethod("setCounter", long.class);
		...
		when(compositeKeyFactory.createKeyForCounter("fqcn", 11L, counterIdMeta)).thenReturn(
				keyComp);
		when(dynamicCompositeKeyFactory.createBaseForQuery(counterMeta, EQUAL)).thenReturn(comp);

		when(counterDao.getCounterValue(keyComp, comp)).thenReturn(150L);

		loader.loadPropertyIntoObject(bean, 11L, dao, counterMeta);

		verify(introspector).setValueToField(bean, setter, 150L);
	...
	public void should_load_join_simple() throws Exception
	{
		...
		DynamicComposite comp = new DynamicComposite();
		when(dynamicCompositeKeyFactory.createBaseForQuery(propertyMeta, EQUAL)).thenReturn(comp);
		...
		verify(mapper).setEagerPropertiesToEntity(eq(120L), eq(columns), eq(joinMeta),
				userCaptor.capture());
		verify(introspector).setValueToField(userCaptor.capture(), eq(idSetter), eq(120L));

		verify(introspector).setValueToField(eq(realObject), eq(idSetter), userCaptor.capture());
	...
	public void should_load_join_list_into_entity() throws Exception
	{
		...
		verify(introspector).setValueToField(realObject, setter, users);
	}
