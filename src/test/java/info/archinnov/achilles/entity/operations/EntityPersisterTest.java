...
public class EntityPersisterTest {
    ...
    public void setUp() throws Exception {
        ...
        entity = CompleteBeanTestBuilder.builder().id(id).name("name").age(52L).addFriends("foo", "bar")
                .addFollowers("George", "Paul").addPreference(1, "FR").buid();
    ...
    public void should_batch_persist_serialVersionUID() throws Exception {
        ...
        ArgumentCaptor<DynamicComposite> compositeCaptor = ArgumentCaptor.forClass(DynamicComposite.class);

        when(helper.findSerialVersionUID(entity.getClass())).thenReturn(151L);
        doNothing().when(dao).insertColumnBatch(eq(id), compositeCaptor.capture(), eq("151"), eq(mutator));
    ...
    public void should_exception_when_serialVersionUID_not_found_while_persisting_entity() throws Exception {
        Map<String, PropertyMeta<?, ?>> propertyMetaMap = new HashMap<String, PropertyMeta<?, ?>>();
        when(entityMeta.getPropertyMetas()).thenReturn(propertyMetaMap);
    ...
    public void should_batch_set_property() throws Exception {
        ...
        when(helper.getValueFromField(entity, anyMethod)).thenReturn(Sets.newHashSet("George", "Paul"));
    ...
    public void should_batch_map_property() throws Exception {
        ...
        verify(dao, times(3)).insertColumnBatch(eq(id), eq(composite), keyValueHolderCaptor.capture(), eq(mutator));
    ...
    public void should_batch_join_entity_when_cascade_persist() throws Exception {
        ...
        when((PropertyMeta) propertyMeta.joinIdMeta()).thenReturn(joinProperties.getEntityMeta().getIdMeta());
    ...
    public void should_not_batch_join_entity_when_no_cascade() throws Exception {
        ...
        when(loader.load(UserBean.class, joinId, (EntityMeta<Long>) joinProperties.getEntityMeta())).thenReturn(
                userBean);
    ...
    public void should_persist_join_entity_when_cascade_persist() throws Exception {
        ...
        verify(dao).insertColumnBatch(eq(joinId), any(DynamicComposite.class), eq("0"), eq(mutator));
        verify(dao).insertColumnBatch(id, composite, joinId.toString(), mutator);
    }
