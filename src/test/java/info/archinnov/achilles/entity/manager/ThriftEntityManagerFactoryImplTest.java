...
public class ThriftEntityManagerFactoryImplTest {
    ...
    public void should_bootstrap() throws Exception {
        ...
        when(entityExplorer.discoverEntities(entityPackages)).thenAnswer(new Answer<List<Class<?>>>() {
            @Override
        ...
        verify(entityParser).fillJoinEntityMeta(eq(keyspace), mapCaptor.capture(), eq(entityMetaMap));
        verify(columnFamilyCreator).validateOrCreateColumnFamilies(eq(entityMetaMap), anyBoolean(), anyBoolean());
    ...
    public void should_bootstrap_with_counter_property() throws Exception {
        ...
        when(entityParser.parseEntity(eq(keyspace), eq(BeanWithSimpleCounter.class))).thenReturn(entityMeta1);
        when(entityMeta1.hasCounter()).thenReturn(true);

        factory.bootstrap();

        verify(entityMetaMap).put(BeanWithSimpleCounter.class, entityMeta1);
        verify(columnFamilyCreator).validateOrCreateColumnFamilies(eq(entityMetaMap), anyBoolean(), eq(true));
    ...
    public void should_create_object_mapper_factory_from_provided_object_mapper() throws Exception {
        ...
        ObjectMapperFactory mapperFactory = ThriftEntityManagerFactoryImpl.factoryFromMapper(mapper);
