...
public class EntityLoaderTest {
    ...
    public void should_load_join_entity_into_object() throws Exception {
        ...
        doNothing().when(helper).setValueToField(any(UserBean.class), eq(idSetter), idCaptor.capture());
        when(interceptorBuilder.build(userBeanCaptor.capture(), eq(entityMeta))).thenReturn(userBean);
    ...
    public void should_load_join_column() throws Throwable {
        ...
        doNothing().when(helper).setValueToField(any(UserBean.class), eq(idSetter), idCaptor.capture());
        when(interceptorBuilder.build(userBeanCaptor.capture(), eq(entityMeta))).thenReturn(userBean);
        doNothing().when(helper).setValueToField(eq(bean), eq(userSetter), userBeanCaptor.capture());
