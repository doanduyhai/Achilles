...
public class ColumnFamilyValidatorTest
{
	...
	{
		when(cfDef.getKeyValidationClass()).thenReturn(LONG_SRZ.getComparatorType().getTypeName());
	...
	{
		when(cfDef.getKeyValidationClass()).thenReturn(INT_SRZ.getComparatorType().getTypeName());
		when((Serializer) entityMeta.getIdSerializer()).thenReturn(LONG_SRZ);
