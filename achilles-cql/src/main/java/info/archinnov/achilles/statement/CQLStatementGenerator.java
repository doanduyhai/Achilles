...
public class CQLStatementGenerator
{
	...
	public Statement generateWhereClauseForSliceQuery(List<String> componentNames,
			List<Object> startValues, List<Object> endValues, BoundingMode boundingMode,
			Select select)
	{
		String componentsDescription = StringUtils.join(componentNames, ",");
		int startIndex = validator.findLastNonNullIndexForComponents(componentsDescription,
				startValues);
		int endIndex = validator
				.findLastNonNullIndexForComponents(componentsDescription, endValues);
		...
		if (startIndex == endIndex)
		{
			...
			buildWhereClauseForLastComponents(componentNames, startValues, endValues, boundingMode,
					startIndex, where);
		...
		else
		{
			...
			buildWhereClauseForLastNonNullComponent(componentNames, startValues, endValues,
					boundingMode, startIndex, endIndex, where);
		}
		return where;
	}

	private void buildWhereClauseForLastComponents(List<String> componentNames,
			List<Object> startValues, List<Object> endValues, BoundingMode boundingMode,
			int startIndex, Where where)
	{
		switch (boundingMode)
		{
	...
	private void buildWhereClauseForLastNonNullComponent(List<String> componentNames,
			List<Object> startValues, List<Object> endValues, BoundingMode boundingMode,
			int startIndex, int endIndex, Where where)
	{
		if (startIndex > endIndex)
		{
			switch (boundingMode)
