...
public class CQLPersisterImpl {
	...
	public void cascadePersist(CQLEntityPersister entityPersister,
			CQLPersistenceContext context, Set<PropertyMeta> joinPMs) {
		Object entity = context.getEntity();
		JoinValuesExtractor extractorAndFilter = new JoinValuesExtractor(entity);

		List<Pair<List<?>, PropertyMeta>> pairList = FluentIterable
				.from(joinPMs).transform(extractorAndFilter)
				.filter(nullJoinValuesFilter).toImmutableList();

		doCascade(entityPersister, context, pairList);
	}

	public void ensureEntitiesExist(CQLPersistenceContext context,
	...
	private void doCascade(CQLEntityPersister entityPersister,
		...
		for (Pair<List<?>, PropertyMeta> pair : pairList) {
			...
			for (Object joinEntity : joinValues) {
				CQLPersistenceContext joinContext = context
						.createContextForJoin(joinPM.getJoinProperties()
