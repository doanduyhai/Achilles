...
public class JoinCollectionAndMapIT
{
    ...
    public void should_persist_join_collection_and_map() throws Exception
    {
        ...
        Row row = session.execute("select friends from EntityWithJoinCollectionAndMap where id=" + bean.getId())
                .one();
        ...
        row = session.execute("select timeline from EntityWithJoinCollectionAndMap where id=" + bean.getId())
                .one();
        Map<Integer, UUID> timelineMap = row.getMap("timeline", Integer.class, UUID.class);
    ...
    public void should_merge_join_collection_and_map() throws Exception
    {
        ...
        Row row = session.execute("select tweets from EntityWithJoinCollectionAndMap where id=" + bean.getId())
                .one();
        List<UUID> tweetsId = row.getList("tweets", UUID.class);
